...
`dist/assets/updates-data.js` の `window.SITE_UPDATES` に1件追加するだけで反映されます。ビルドは不要です。
`date` は実際の更新日（日本時間、`YYYY-MM-DD`）、`category` はカテゴリ、`text` は更新内容、`href` は任意の関連ページへの相対リンクです。

```js
{ date: "2026-09-24", category: "クエスト", text: "クエストに〇〇を追加しました", href: "quests.html" }
```

上記は書式例です。実際に更新した内容に置き換えてください。日付の新しい順に自動整列し、同日の項目はデータの記載順で表示します。ホームは最新5件、6件以上の場合は「更新履歴をすべて見る」でその場に全件展開できます。既存の履歴は残して追加してください。HTMLタグは使わず、文章として入力します。サイトの変更内容を自動検出する方式ではありません。
