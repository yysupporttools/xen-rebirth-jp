"use strict";
(()=>{const $=id=>document.getElementById(id),cfg=window.XEN_GLOSSARY_CONFIG||{},db=(window.supabase&&cfg.SUPABASE_URL&&cfg.SUPABASE_ANON_KEY)?window.supabase.createClient(cfg.SUPABASE_URL,cfg.SUPABASE_ANON_KEY):null;
const JST='Asia/Tokyo', now=new Date(), jstNow=new Date(now.toLocaleString('en-US',{timeZone:JST})); let cursor=new Date(jstNow.getFullYear(),jstNow.getMonth(),1),events=[];
...
async function load(){if(!db){$('calendar-status').textContent='カレンダー設定を読み込めませんでした。';render();return} $('calendar-status').textContent='イベント情報を読み込み中…';const {data,error}=await db.from('xen_events').select('*').order('start_time',{ascending:true});if(error){$('calendar-status').textContent='イベント情報を取得できませんでした：'+error.message;render();return}events=data||[];$('calendar-status').textContent=events.length?`${events.length}件のイベント情報を表示しています。`:'イベント情報はまだ登録されていません。';render();const requested=new URL(location.href).searchParams.get('event');if(requested){const selected=events.find(e=>String(e.id)===requested);if(selected){const d=new Date(selected.start_time);if(!Number.isNaN(d.getTime())){const jst=new Date(d.toLocaleString('en-US',{timeZone:JST}));cursor=new Date(jst.getFullYear(),jst.getMonth(),1);render()}show(selected)}else{$('calendar-status').textContent='指定のイベントが見つかりません。カレンダーからお探しください。'}}}
