...
(() => {
  ...
  function tick() {
    ...
    if(key!==signature) {
      signature=key;
      document.querySelectorAll('[data-next-boss]').forEach(el=>{el.innerHTML=next?`<p class="location">${escape(next.location)}</p><h2>${escape(next.name)}</h2><div class="countdown" data-countdown="${next.start}" aria-label="出現予定までの残り時間"></div><p>${dateFormat.format(next.start)} JST</p><p class="small">公式予定 / 実際の出現状況は未取得</p>`:'<h2>最新の予定を確認</h2><p>収録された次回予定がありません。公式タイマーでご確認ください。</p>';});
