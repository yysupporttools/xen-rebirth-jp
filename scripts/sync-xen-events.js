...
/* Xen Rebirth Event Date/Time Parser Ver.7
...
(async()=>{
 console.log("=== Xen Rebirth Event Date/Time Parser Ver.7 ===");
 console.log("DRY RUN / NO SUPABASE WRITES");
 const events=discover(await get(CALENDAR_URL)).slice(0,MAX_EVENTS);let ok=0,review=0,timed=0,allDay=0;
 for(let i=0;i<events.length;i++){const e=events[i],html=await get(e.url),title=titleOf(html,e.title),rec=preview({...e,title},headerWindow(html,title));if(rec.status==="OK")ok++;else review++;if(rec.all_day===true)allDay++;else if(rec.all_day===false)timed++;console.log(`\n===== PREVIEW ${i+1}/${events.length} =====`);console.log(JSON.stringify(rec,null,2));await new Promise(r=>setTimeout(r,250));}
 console.log("\n=== SUMMARY ===");console.log(`Unique events:          ${events.length}`);console.log(`Parsed OK:              ${ok}`);console.log(`Needs review:           ${review}`);console.log(`Timed events:           ${timed}`);console.log(`Date-only events:       ${allDay}`);console.log("Supabase writes:        0");console.log("IMPORTANT: Dry run only; no database rows were changed.");
